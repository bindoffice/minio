...
import (
	...
	"github.com/bindoffice/bind-store/cmd/config/dns"
...
func (a adminAPIHandlers) AddUser(w http.ResponseWriter, r *http.Request) {
	...
	if err = globalIAMSys.CreateUser(accessKey, ureq); err != nil {
		writeErrorResponseJSON(ctx, w, toAdminAPIErr(ctx, err), r.URL)
		return
	}
...
func (a adminAPIHandlers) UpdateServiceAccount(w http.ResponseWriter, r *http.Request) {
	...
	_, _, err := globalIAMSys.GetServiceAccount(ctx, accessKey)
	if err != nil {
		writeErrorResponseJSON(ctx, w, toAdminAPIErr(ctx, err), r.URL)
		return
	}

	// CVE-2024-24747: only callers with explicit admin:UpdateServiceAccount may edit
	// access keys. Do not allow parent-user or self-service implicit bypass.
	if !globalIAMSys.IsAllowed(iampolicy.Args{
		AccountName:     cred.AccessKey,
